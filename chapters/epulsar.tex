...
There has been prior work in building edge-centric pub-sub systems, which include EMMA \cite{emma}, FogMQ \cite{DBLP:journals/corr/AbdelwahabH16a}, and MutiPub \cite{multipub}.  However, these systems do not meet the data communication and/or the scalability needs of the aforementioned applications.  EMMA does not handle message reliability guarantees or at-least-once/exactly-once semantics that are typically offered by commercial pub-sub systems. FogMQ relies on creating a clone in the proximity of each device to handle communication on behalf of that device. % Such an architecture resembles a geo-distributed pub-sub system, albeit with one broker entity per client. 
