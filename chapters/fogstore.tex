\chapter{FogStore: A Geo-Distributed Key-Value Store Guaranteeing Low
...
\State $replicas \gets \{\}$
\For{$it \in iterate(H, it_S)$}
	\If{$encodingDist (it.tile\_id, itemToken.tile\_id) \leq inCoiDist \quad \& \quad it.key \geq itemToken.key \quad \& \quad it \text{ not already chosen}$}
