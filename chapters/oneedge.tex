...
\subsubsection{Response Time of Application Component}
...
\caption{\oneedge{}'s System Architecture. A central controller (left blow-up) coordinates with all the Edge sites (right blow-up).\todo{Needs update}}
...
\subsection{Edge Site}
An Edge site consists of three main components, as shown in the right blow-up in \cref{fig:system_arch} - the Site Agent, Container Runtime and Worker Agent.% and the Monitoring Subsystem. 
...
\item In addition to having applied updates that have not yet been successfully applied on Edge sites, the Aggregate State is also eventually consistent with the ground-truth resource allocation that is maintained by the Site Agents. Since Site Agents are able to update allocation state of the site without coordinating with the Controller, the Controller's Aggregate State does not see those autonomous updates until the state is synchronized with the Edge site, that is done periodically.
\end{itemize}
\end{comment}
...
\State $L_{client} \gets A.num\_levels-1$ \Comment{Pipeline level of the client}
\State $lat_{proc}^{total} \gets c.proc\_latency()$ \Comment{Total processing latency consumed so far}
...
\If{$L_w < 0$} \Comment{Search is complete}
    \State \Return []
\EndIf
\State $L \gets A.level\_cfgs [L_w]$
\State $appid \gets A.app\_id$
\State $\mathcal{M} \gets \mathcal{A}.client\_mapping$ \Comment{Mapping of clients to workers}
...
\For{$c \in reuse\_candidates$}
    \If{$c.curr\_clients < L.max\_clients$}
        ...
        \If{$lat_{response} < L.response\_time\_threshold$}
...
\State $E \gets \{\}$ \Comment{Dictionary to hold processing latencies for each worker}
\For{$m \in MD$}
    \If{$m \notin E$}
...
\For{$c \in clients$}
    ...
    \State $level \gets APP.num\_levels-2$ \Comment{Starting from parent of client}
    \While{$level \geq 0$} \Comment{Iterating over all downstream levels of app pipeline}
        ...
        \If{$lat_{response} \geq L.response\_time\_threshold$}
...
\While{$level \geq l_{viol}$} \Comment{Collect all component latencies through level $l_{viol}$}
    ...
    \State $level \gets level - 1$
\EndWhile
\State $E_c  [\left( c, feedback \right) ]\gets \dist \left( NC_w[b_{curr}], NC_{c}[b_{curr}]\right)/2$ \Comment{NW latency of feedback from violated level}
\State $lat_{response}^{curr} \gets \sum_{\left( w, l\right) \in E_c}{E_c[\left( w, l\right)][b_{curr}]}$   \Comment{Current response time at $l_{viol}$}
\State $b_{pre} \gets find\_pre\_violation\_bucket \left( E_c, l_{viol}, response\_time\_threshold, b_{curr}, B \right)$
\State $lat_{response}^{pre} \gets \sum_{\left( w, l\right) \in E_c}{E_c[\left( w, l\right)][b_{pre}]}$    \Comment{response time at $l_{viol}$ before violation}
\State $change \gets \{\}$  \Comment{Dict to store relative change in component latency wrt change in response time}
...
\While{$lat_{response} > response\_time\_threshold$}
